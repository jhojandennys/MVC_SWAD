        ...
        <tbody class="min-w-full border-y-0">
            ...
            <tr class="bg-neutral-3000  text-center py-2  border-stone-500 border-y">
                ...
                <td class="text-xl flex justify-center items-center gap-2 "             
                    data-id="<%= usr.getId()%>"
                    data-dni="<%= usr.getDni()%>"
                    data-nombre="<%= usr.getNombres()%>"
                    data-apellido-pat="<%= usr.getApePaterno()%>"
                    data-apellido-mat="<%= usr.getApeMaterno()%>"
                    data-correo="<%= usr.getCorreo()%>"
                    data-pass="<%= usr.getPassword()%>"
                    data-telefono="<%= usr.getTelefono()%>"
                    data-rolNombre="<%= usr.getRol().getNombre()%>"
                    data-rol="<%= usr.getRol().getId()%>">
    ...
    <div class="modal bg-secondary-800 p-4 md:p-6 rounded-md text-white w-[95%] max-w-[65rem] relative">
        ...
        <form action="/Users/add" method="post" class="text-white my-6 flex gap-y-4  md:gap-y-6 flex-col xs:flex-row  flex-wrap w-full">
            ...
            <div class="flex flex-col gap-y-4 md:gap-y-6 w-full md:w-2/4  ">
                ...
                <div class="flex flex-col xs:flex-row gap-0.5 justify-between  xs:items-center  w-full ">
                    <label for="crol" class="text-lg font-medium xs:w-2/4 sm:w-2/5">Rol:</label>
                    <select  name="rol" id="crol" class="bg-secondary-800 outline-none px-2 py-1 font-sans w-full 
    ...
    <div class="modal bg-secondary-800 p-4 md:p-6 rounded-md text-white w-[95%] max-w-[65rem] relative">
        ...
        <form action="/Users/edit" method="post" class="text-white my-6 flex gap-y-4  md:gap-y-6 flex-col xs:flex-row  flex-wrap w-full">
            ...
            <div class="flex flex-col gap-y-4 md:gap-y-6 w-full md:w-2/4  ">
                ...
                <div class="flex flex-col xs:flex-row gap-0.5 justify-between  xs:items-center  w-full ">
                    <label for="erol" class="text-lg font-medium xs:w-2/4 sm:w-2/5">Rol:</label>
                    <select  name="rol" id="erol" class="bg-secondary-800 outline-none px-2 py-1 font-sans w-full 
                             border  border-secondary-300 rounded-md " >
                        <%
                            if (roles != null) {
                                for (Rol r : roles) {
